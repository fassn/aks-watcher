...
export const DeleteAll = ({ setFlash }: DeleteAllProps) => {
    ...
    return (
        ...
        <button className="flex items-center mr" onClick={showModal} id="delete_all">
        ...
        <Modal show={modalShow} onRequestClose={closeModal} className="w-80 h-48">
            ...
            <div className="flex h-1/2">
                <button onClick={onClick} className="flex-none w-full justify-center self-end bg-deep-blue text-cream font-semibold py-2 px-4 border border-blue-500 hover:border-transparent rounded">Confirm</button>
            </div>
        </Modal>
    </div>
    )
